...
<body class="bg-blue-500">
        ...
        <div class="h-screen bg-opacity-50 bg-black flex items-center justify-center"
            ...
            <div class="mx-2 text-center">
                <h1 class="text-gray-100 font-extrabold text-4xl xs:text-5xl md:text-6xl">
                    <span class="text-white">We're determined for your better life.
                </h1>
                <p class="text-gray-200 font-extrabold text-xl xs:text-2xl md:text-3xl leading-tight">
                     You can get 
                     <span class="text-white">
                        the care you need 24/7 – be it online or in person.
                    </span> and 
                    <span class="text-white">You will be treated by caring specialist doctors.</span>
                </p>
                <div class="inline-flex">
                    <button
                        class="p-2 my-5 mx-2 bg-indigo-700 hover:bg-indigo-800 font-bold text-white rounded border-2 border-transparent hover:border-indigo-800 shadow-md transition duration-500 md:text-xl">Lire Plus
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="flex justify-center items-center space-x-4 mt-4">
        <form action="{{ route('index') }}" method="GET" class="flex gap-2">
            @csrf
            @method('GET')
            <select id="countries" name="speciality"
                ...
                <option selected disabled>Selectionnez une spécialité</option>
                <option value="Tout">All</option>
                @foreach ($specialities as $s)
                    <option value="{{ $s->id }}">{{ $s->specialityName }}</option> @endforeach
            </select>
            <button type="submit"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16 max-md:max-w-lg mx-auto">
        @foreach ($medecin as $m)
            <div
                ...
                <div class="p-6">
                    <h3 class="text-xl font-bold text-[#333] text-black"> {{ $m->user->name }} </h3>
                    <hr class="my-6" />
                    {{-- <p class="text-gray-400 text-sm"> {{ $m->speciality->specialityName }} </p> --}}
                    <a href="{{ route('doctor_profile.show', $m->id) }}"
                        class="inline-flex items-center px-3 py-2 my-3 text-sm font-medium
