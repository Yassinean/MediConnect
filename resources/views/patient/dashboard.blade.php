...
<body class="bg-blue-500">
        ...
        <form action="{{ route('index') }}" method="GET" class="flex gap-2">
            ...
            <select id="countries" name="speciality"
                ...
                @foreach ($specialities as $s)
                    <option value="{{ $s->id }}">{{ $s->specialityName }}</option>
                @endforeach
            </select>
            <button type="submit"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16 max-md:max-w-lg mx-auto">
        @foreach ($medecin as $m)
            <div
                ...
                <div class="p-6">
                    <h3 class="text-xl font-bold text-[#333] text-black"> {{ $m->name }} </h3>
                    <hr class="my-6" />
                    <p class="text-gray-400 text-sm"> {{ $m->specialityName }} </p>
                    <a href="{{ route('doctor_profile', ['id' => $m->id]) }}"
    ...
    <div class="max-w-md mx-auto mt-10 bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <form class="py-4 px-6" action="" method="POST">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2" for="speciality">
                    Specialité
                </label>
                <select
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="speciality" name="specialityAp">
            ...
            <div class="flex items-center justify-center mb-4">
                <form action="{{route('doctor_profile')}}" method="POST">
                    <button
